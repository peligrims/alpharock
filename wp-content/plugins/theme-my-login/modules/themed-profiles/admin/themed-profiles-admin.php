...
class Theme_My_Login_Themed_Profiles_Admin extends Theme_My_Login_Abstract {
	...
	public function settings_field_themed_profiles() {
		...
		foreach ( $wp_roles->get_names() as $role => $role_name ) {
			...
			<?php 
			if ($role_name=="Administrator") $role_name="Администратор";
			if ($role_name=="Author") $role_name="Автор";
			if ($role_name=="Contributor") $role_name="Участник";
			if ($role_name=="Editor") $role_name="Редактор";
			if ($role_name=="Subscriber") $role_name="Подписчик";
			?>
